...
const StoreDetail = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      await axios.get(`${url}/store/${id}`, { withCredentials: true }).then((resp) => {
        // console.log('store',resp.data);
        if (resp.data.error) return setOpenError({ open: true, message: 'Error Occurred' });
        setStore(resp.data);
        setToStoreForm({name:resp.data.name,type:resp.data.type,date:resp.data.date,description:resp.data.description,quantity:resp.data.quantity})
      }).catch((error) => {
...
//   console.log(toStoreForm);
  ...
  const sendToStore = async(e)=>{
    ...
    // console.log(toStoreForm);
    await axios.put(`${url}/store/${id}`,toStoreForm,{withCredentials:true}).then((resp)=>{
        if(resp.data.error) return setOpenError({open:true,message:`${resp.data.error}`})
        console.log('New Data',resp.data);
        setStore(resp.data)
        setOpenSuccess({open:true,message:"Successfully Updated The Item"})
        closeToStore()
    })
  ...
  return (
    <>
      <PageTitle>store Detail | {store.name}</PageTitle>
      ...
      <TableContainer>
        ...
        {/* End of delete Section */}
          ...
          <ModalHeader>Store Update</ModalHeader>
          <ModalBody>
...
<div className="flex flex-wrap ">
  ...
  <div className="w-full mx-4 my-4 bg-white dark:bg-gray-700 shadow-lg rounded-lg overflow-hidden">
    ...
    <div className="px-4 py-2">
      ...
      <p className="text-gray-800 dark:text-gray-100 ml-4 mb-2">{store.type}</p>
        </div>

        <div className='flex'>
      <p className="text-sm text-gray-600 dark:text-gray-100 font-bold mb-2">Quantity</p>
      <p className="text-gray-800 dark:text-gray-100 ml-4 mb-2">{store.quantity}</p>
        </div>

        <div className='flex'>
      <p className="text-sm text-gray-600 dark:text-gray-100 font-bold mb-2">Remaining Quantity</p>
      <p className="text-gray-800 dark:text-gray-100 ml-4">{store.remaining}</p>
        </div>
    </div>
