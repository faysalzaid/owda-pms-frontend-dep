...
const BankDetail = () => {
    ...
    function openModal(){
      ...
      useEffect(()=>{
        const getData=async()=>{
            await axios.get(`${url}/banks/${id}`,{withCredentials:true}).then((resp)=>{
                if(resp.data.error){
                    setOpenError({open:true,message:`${resp.data.error}`})
                }else{

                    setBank(resp.data)
                    setBankForm({name:resp.data.name})
                }
      ...
      const handleSubmit = async(e) => {
        e.preventDefault();
        await axios.put(`${url}/banks/${id}`,bankForm,{withCredentials:true}).then((resp)=>{
          if(resp.data.error){
            setOpenError({open:true,message:`${resp.data.error}`})
          }else{
      ...
      useEffect(()=>{
        setFetchedResult(searchTerm.length<1?bank:searchResult)
      },[bank,searchTerm])
  ...
  const handleDelete = async()=>{
    await axios.delete(`${url}/banks/${id}`,{withCredentials:true}).then((resp)=>{
        const data = bank.filter((dt)=>dt.id!==isDeleteOpen.id)
        setBank(data)
    ...
    return (
      <>
        ...
        <PageTitle>Bank | {bank.name}</PageTitle>
      ...
      {/* Delete Confirm section */}
        ...
        <Button onClick={openModal} className="custom-button">Update Bank</Button>
  ...
  <div className="bg-gray-100 dark:bg-gray-800 text-dark-900 dark:text-gray-300 shadow-md rounded-lg px-8 pt-6 pb-8 mb-4 flex flex-wrap">
      <div className="w-full md:w-1/2 mb-4">
            <h2 className="text-lg font-semibold mb-4">Bank Details</h2>
                <div className="mb-4">
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-300">Bank Name</p>
                  <p className="text-sm font-semibold dark:text-gray-300">{bank.name}</p>
                </div>
      </div>
    </div>
  </div>
