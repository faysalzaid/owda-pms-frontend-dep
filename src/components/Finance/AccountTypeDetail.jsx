...
const AccountTypeDetail = () => {
    ...
    function openModal(){
      ...
      useEffect(()=>{
        const getData=async()=>{
            await axios.get(`${url}/accountTypes/${id}`,{withCredentials:true}).then((resp)=>{
                if(resp.data.error){
                    setOpenError({open:true,message:`${resp.data.error}`})
                }else{

                    setAccountType(resp.data)
                    setAccountTypeForm({name:resp.data.name})
                }
      ...
      const handleSubmit = async(e) => {
        e.preventDefault();
        await axios.put(`${url}/accountTypes/${id}`,accountTypeForm,{withCredentials:true}).then((resp)=>{
          if(resp.data.error){
            setOpenError({open:true,message:`${resp.data.error}`})
          }else{
      ...
      useEffect(()=>{
        setFetchedResult(searchTerm.length<1?accountType:searchResult)
  ...
  const handleDelete = async()=>{
    await axios.delete(`${url}/accountTypes/${id}`,{withCredentials:true}).then((resp)=>{
        const data = accountType.filter((dt)=>dt.id!==isDeleteOpen.id)
        setAccountType(data)
    ...
    return (
      <>
        ...
        <PageTitle>accountType | {accountType.name}</PageTitle>
      ...
      <ModalHeader>Update accountType</ModalHeader>
      <ModalBody>
  ...
  <div className="w-full">
    <div className="bg-gray-100 dark:bg-gray-800 text-dark-900 dark:text-gray-300 shadow-md rounded-lg px-8 pt-6 pb-8 mb-4 flex flex-wrap">
      <div className="w-full md:w-1/2 mb-4">
            <h2 className="text-lg font-semibold mb-4">accountType Details</h2>
                <div className="mb-4">
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-300">accountType Name</p>
                  <p className="text-sm font-semibold">{accountType.name}</p>
                </div>
      </div>
    </div>
  </div>
