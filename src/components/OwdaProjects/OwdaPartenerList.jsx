...
const OwdaPartenerList = () => {
    ...
    const [partenerForm,setPartenerForm] = useState({
        name:"",
    ...
    function openModal(){
      ...
      const handleSubmit = async(e) => {
        e.preventDefault();
        await axios.post(`${url}/partener`,partenerForm,{withCredentials:true}).then((resp)=>{
          if(resp.data.error){
            setOpenError({open:true,message:`${resp.data.error}`})
          }else{
            // console.log(resp.data);
            setPartener((prev)=>[...prev,resp.data])
      ...
      useEffect(()=>{
        setFetchedResult(searchTerm.length<1?partener:searchResult)
      },[partener,searchTerm])
    ...
    const searchHandler = async(search)=>{
      ...
      if(search!==0){
        const newPayroll = partener?.filter((empl)=>{
  ...
  const handleDelete = async()=>{
    await axios.delete(`${url}/partener/${isDeleteOpen.id}`,{withCredentials:true}).then((resp)=>{
        const data = partener.filter((dt)=>dt.id!==isDeleteOpen.id)
        setPartener(data)
    ...
    return (
      ...
      {/* Search section */}
        ...
        <div className="relative">
            ...
            dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search partener..." required />
        </div>
      ...
      {/* Delete Confirm section */}
        ...
        <Button onClick={openModal} className="custom-button">New partener</Button>
  
      
        </TableContainer>

        <Modal isOpen={isOpen} onClose={closeModal}>
      <ModalHeader>Register partener</ModalHeader>
      <ModalBody>
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          <Label>
            ...
            //   type="number"
              className="mt-1"
              name="name"
              onChange={(e)=>setPartenerForm({...partenerForm,name:e.target.value})}
              required
            />
          </Label>
...
<Button block size="large" type="submit" className="custom-button">
      ...
      <Table>
        <TableHeader>
          <TableRow>
            <TableCell className="font-semibold">partener</TableCell>
...
export default OwdaPartenerList