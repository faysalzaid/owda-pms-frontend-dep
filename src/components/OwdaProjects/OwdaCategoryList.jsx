...
const OwdaCategoryList = () => {
    ...
    function openModal(){
      ...
      const handleSubmit = async(e) => {
        e.preventDefault();
        await axios.post(`${url}/category`,categoryForm,{withCredentials:true}).then((resp)=>{
          if(resp.data.error){
      ...
      useEffect(()=>{
        setFetchedResult(searchTerm.length<1?category:searchResult)
      },[category,searchTerm])
    ...
    const searchHandler = async(search)=>{
      ...
      if(search!==0){
        const newPayroll = category?.filter((empl)=>{
  ...
  const handleDelete = async()=>{
    await axios.delete(`${url}/category/${isDeleteOpen.id}`,{withCredentials:true}).then((resp)=>{
        const data = category.filter((dt)=>dt.id!==isDeleteOpen.id)
        setCategory(data)
    ...
    return (
      ...
      {/* Search section */}
        ...
        <div className="relative">
            ...
            dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search category..." required />
        </div>
      ...
      {/* Delete Confirm section */}
        ...
        <Button onClick={openModal} className="custom-button">New Category</Button>
  
      
        </TableContainer>

        <Modal isOpen={isOpen} onClose={closeModal}>
      <ModalHeader>Register Category</ModalHeader>
      <ModalBody>
...
export default OwdaCategoryList