...
const OwdaSitesList = () => {
    ...
    function openModal(){
      ...
      const handleSubmit = async(e) => {
        e.preventDefault();
        await axios.post(`${url}/site`,sitesForm,{withCredentials:true}).then((resp)=>{
          if(resp.data.error){
            setOpenError({open:true,message:`${resp.data.error}`})
          }else{
            // console.log(resp.data);
            setSites((prev)=>[...prev,resp.data])
      ...
      useEffect(()=>{
        setFetchedResult(searchTerm.length<1?sites:searchResult)
      },[sites,searchTerm])
  ...
  const handleDelete = async()=>{
    await axios.delete(`${url}/site/${isDeleteOpen.id}`,{withCredentials:true}).then((resp)=>{
        const data = sites.filter((dt)=>dt.id!==isDeleteOpen.id)
        setSites(data)
    ...
    return (
      ...
      {/* Search section */}
        ...
        <div className="relative">
            ...
            dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search Sites..." required />
        </div>
      ...
      {/* Delete Confirm section */}
        ...
        <Button onClick={openModal} className="custom-button">New Site</Button>
  
      
        </TableContainer>

        <Modal isOpen={isOpen} onClose={closeModal}>
      <ModalHeader>Register Site</ModalHeader>
      <ModalBody>
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          <Label>
            ...
            //   type="number"
              className="mt-1"
              name="name"
              onChange={(e)=>setSitesForm({...sitesForm,name:e.target.value})}
              required
            />
          </Label>
...
<Button block size="large" type="submit" className="custom-button">
      ...
      <Table>
        <TableHeader>
          <TableRow>
            <TableCell className="font-semibold">Sites</TableCell>
