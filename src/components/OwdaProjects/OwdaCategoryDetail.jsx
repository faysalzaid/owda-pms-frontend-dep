...
const OwdaCategoryDetail = () => {
    ...
    function openModal(){
      ...
      useEffect(()=>{
        const getData=async()=>{
            await axios.get(`${url}/category/${id}`,{withCredentials:true}).then((resp)=>{
                if(resp.data.error){
                    setOpenError({open:true,message:`${resp.data.error}`})
                }else{

                    setCategory(resp.data)
                    setCategoryForm({name:resp.data.name})
                }
      ...
      const handleSubmit = async(e) => {
        e.preventDefault();
        await axios.put(`${url}/category/${id}`,categoryForm,{withCredentials:true}).then((resp)=>{
          if(resp.data.error){
            setOpenError({open:true,message:`${resp.data.error}`})
          }else{
  ...
  const handleDelete = async()=>{
    await axios.delete(`${url}/category/${id}`,{withCredentials:true}).then((resp)=>{
    ...
    return (
      ...
      {/* Delete Confirm section */}
        ...
        <Button onClick={openModal}>Update category</Button>
...
export default OwdaCategoryDetail