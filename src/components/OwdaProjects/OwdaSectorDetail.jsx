...
const OwdaSectorDetail = () => {
    ...
    function openModal(){
      ...
      useEffect(()=>{
        const getData=async()=>{
            await axios.get(`${url}/sector/${id}`,{withCredentials:true}).then((resp)=>{
                if(resp.data.error){
                    setOpenError({open:true,message:`${resp.data.error}`})
                }else{

                    setSector(resp.data)
                    setSectorForm({name:resp.data.name})
                }
      ...
      const handleSubmit = async(e) => {
        e.preventDefault();
        await axios.put(`${url}/sector/${id}`,sectorForm,{withCredentials:true}).then((resp)=>{
          if(resp.data.error){
            setOpenError({open:true,message:`${resp.data.error}`})
          }else{
      ...
      useEffect(()=>{
        setFetchedResult(searchTerm.length<1?sector:searchResult)
      },[sector,searchTerm])
  ...
  const handleDelete = async()=>{
    await axios.delete(`${url}/sector/${id}`,{withCredentials:true}).then((resp)=>{
        const data = sector.filter((dt)=>dt.id!==isDeleteOpen.id)
        setSector(data)
    ...
    return (
      <>
        ...
        <PageTitle>Sector | {sector.name}</PageTitle>
      ...
      {/* Delete Confirm section */}
        ...
        <Button onClick={openModal} className="custom-button">Update sector</Button>
...
export default OwdaSectorDetail