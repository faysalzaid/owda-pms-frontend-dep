...
const OwdaBudgetCategoryList = () => {
    ...
    function openModal(){
      ...
      const handleSubmit = async(e) => {
        ...
        await axios.post(`${url}/budgetCategory`,budgetCategoryForm,{withCredentials:true}).then((resp)=>{
          if(resp.data.error){
            setOpenError({open:true,message:`${resp.data.error}`})
          }else{
            // console.log(resp.data);
            setBudgetCategory((prev)=>[...prev,resp.data])
      ...
      useEffect(()=>{
        setFetchedResult(searchTerm.length<1?budgetCategory:searchResult)
      },[budgetCategory,searchTerm])
  
  
    const searchHandler = async(search)=>{
      setSearchTerm(search)
      if(search!==0){
        const newData = budgetCategory?.filter((empl)=>{
  ...
  const handleDelete = async()=>{
    await axios.delete(`${url}/budgetCategory/${isDeleteOpen.id}`,{withCredentials:true}).then((resp)=>{
        const data = budgetCategory.filter((dt)=>dt.id!==isDeleteOpen.id)
    ...
    return (
      <>
        ...
        <TitleChange name={`BLC | ${settings.name}`}/>
      ...
      {/* Search section */}
        ...
        <div className="relative">
            ...
            dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search BL Category..." required />
        </div>
      ...
      {/* Delete Confirm section */}
        ...
        <Button onClick={openModal} className="custom-button">New Budget Category</Button>
  
      
        </TableContainer>

        <Modal isOpen={isOpen} onClose={closeModal}>
      <ModalHeader>Register Budget Category</ModalHeader>
      <ModalBody>
            ...
            <Fragment key={i}>
              <TableRow>
                ...
                <TableCell className="flex justify-center space-x-2">
                  <Link to={`/app/budgetCategory/${row.id}`}>
...
export default OwdaBudgetCategoryList